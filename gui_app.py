...
class SmartInternetTroubleshooter(tk.Tk):
    ...
    def _configure_styles(self):
        ...
        style.map(
            "Dark.TButton",
            background=[("active", "#2b3240"), ("disabled", "#151922")],
        ...
        style.map("Accent.TButton", background=[("active", "#12345d"), ("disabled", "#151922")])
        style.configure(
            "Danger.TButton",
            background=self.colors["red_bg"],
            foreground="#fecaca",
            bordercolor="#7f1d1d",
            padding=(10, 6),
        )
        style.map("Danger.TButton", background=[("active", "#3b1218"), ("disabled", "#151922")])
        style.configure(
            "Dark.TEntry",
    ...
    def _section_title(self, parent, text):
        return self._tk_label(parent, text=text, font=("Segoe UI", 12, "bold"))
    ...
    def _tool_button(self, parent, text, command, style="Dark.TButton"):
        return ttk.Button(parent, text=text, command=command, style=style)

    def _stat_card(self, parent, title, value_var, color, bg, column):
        card = self._card(parent, bg=bg)
        card.grid(row=0, column=column, sticky=tk.NSEW, padx=(0, 10))
        card.columnconfigure(0, weight=1)
        self._tk_label(card, text=title.upper(), fg=color, bg=bg, font=("Segoe UI", 8, "bold")).grid(
            row=0,
            column=0,
            sticky=tk.W,
            padx=14,
        ...
        value = self._tk_label(card, textvariable=value_var, fg=color, bg=bg, font=("Segoe UI", 22, "bold"))
        value.grid(row=1, column=0, sticky=tk.W, padx=14, pady=(0, 12))
        return card, value
    ...
    def _build_window(self):
        ...
        ).pack(anchor=tk.W, padx=16, pady=(18, 24))

        nav_items = [
            ("Dashboard", self.refresh_dashboard_stats),
            ("Company Devices", self.load_devices),
            ("Network Discovery", self.discover_network_devices),
            ("Internet Quality", self.internet_quality_test),
            ("Logs / Reports", self.open_logs_folder),
        ]
        for text, command in nav_items:
            button = self._tool_button(sidebar, text, command)
            button.pack(fill=tk.X, padx=12, pady=4)

        sidebar_footer = tk.Frame(sidebar, bg=self.colors["sidebar"])
        sidebar_footer.pack(side=tk.BOTTOM, fill=tk.X, padx=16, pady=16)
        self._muted_label(sidebar_footer, text="Version 1.0").pack(anchor=tk.W)
        self._muted_label(sidebar_footer, text="Network Monitor").pack(anchor=tk.W, pady=(2, 0))
        ...
        topbar = tk.Frame(main, bg=self.colors["bg"], highlightbackground="#1b1f27", highlightthickness=1)
        topbar.grid(row=0, column=0, sticky=tk.EW)
        topbar.columnconfigure(1, weight=1)
        self._tk_label(topbar, text="System Online", fg=self.colors["green"], bg=self.colors["bg"]).grid(
            row=0,
            column=0,
            sticky=tk.W,
            padx=22,
        ...
        self.network_health_top_label.grid(row=0, column=2, sticky=tk.E, padx=(0, 24))
        ...
        self.internet_quality_top_label.grid(row=0, column=3, sticky=tk.E, padx=(0, 22))

        content = tk.Frame(main, bg=self.colors["bg"])
        content.grid(row=1, column=0, sticky=tk.NSEW, padx=22, pady=18)
        ...
        ttk.Label(header, text="Dashboard", style="Title.TLabel").grid(row=0, column=0, sticky=tk.W)
        self._muted_label(header, text="Network status overview and manual troubleshooting").grid(
            row=1,
            column=0,
            sticky=tk.W,
            pady=(2, 0),
        )
        self._tool_button(header, "Refresh", self.refresh_dashboard_stats, style="Accent.TButton").grid(
            row=0,
            column=1,
            rowspan=2,
            sticky=tk.E,
        ...
        stats = tk.Frame(content, bg=self.colors["bg"])
        stats.grid(row=1, column=0, sticky=tk.EW, pady=(0, 14))
        for column in range(5):
            stats.columnconfigure(column, weight=1)
        self._stat_card(stats, "Total Devices", self.total_devices_var, self.colors["text"], self.colors["panel"], 0)
        self._stat_card(stats, "OK Devices", self.ok_devices_var, self.colors["green"], self.colors["green_bg"], 1)
        self._stat_card(stats, "Down Devices", self.down_devices_var, self.colors["red"], self.colors["red_bg"], 2)
        self._stat_card(stats, "Unknown", self.unknown_devices_var, self.colors["orange"], self.colors["orange_bg"], 3)
        _, self.internet_quality_card_value = self._stat_card(
            stats,
            "Internet Quality",
            self.internet_quality_score_var,
            self.colors["blue"],
            self.colors["blue_bg"],
            4,
        )

        inputs = self._card(content)
        inputs.grid(row=2, column=0, sticky=tk.EW, pady=(0, 12))
        inputs.columnconfigure(1, weight=1)
        inputs.columnconfigure(3, weight=1)
        inputs.columnconfigure(5, weight=2)
        self._section_title(inputs, "Network Inputs").grid(row=0, column=0, columnspan=6, sticky=tk.W, padx=14, pady=(12, 8))

        self._muted_label(inputs, text="Host / Domain").grid(row=1, column=0, sticky=tk.W, padx=(14, 8), pady=(0, 12))
        ttk.Entry(inputs, textvariable=self.host_var, style="Dark.TEntry").grid(
            row=1,
            column=1,
            sticky=tk.EW,
            padx=(0, 14),
        ...
        ttk.Entry(inputs, textvariable=self.port_var, width=10, style="Dark.TEntry").grid(
            row=1,
            column=3,
            sticky=tk.EW,
            padx=(0, 14),
        ...
        ttk.Entry(inputs, textvariable=self.scan_ports_var, style="Dark.TEntry").grid(
            row=1,
            column=5,
            sticky=tk.EW,
            padx=(0, 14),
        ...
        tools = self._card(content)
        tools.grid(row=3, column=0, sticky=tk.EW, pady=(0, 12))
        for column in range(6):
            tools.columnconfigure(column, weight=1)
        self._section_title(tools, "Basic Network Tools").grid(row=0, column=0, columnspan=6, sticky=tk.W, padx=14, pady=(12, 2))
        self._muted_label(tools, text="Run manual Windows-focused checks without changing the saved device list.").grid(
            row=1,
            column=0,
            columnspan=6,
            sticky=tk.W,
        ...
        for index, (text, command) in enumerate(button_specs):
            style = "Accent.TButton" if text in ("Internet Troubleshooter", "Internet Quality Test") else "Dark.TButton"
            button = self._tool_button(tools, text, command, style=style)
            button.grid(row=2 + index // 6, column=index % 6, sticky=tk.EW, padx=6, pady=5)

        devices = self._card(content)
        devices.grid(row=4, column=0, sticky=tk.EW, pady=(0, 12))
        devices.columnconfigure(1, weight=1)
        devices.columnconfigure(3, weight=1)
        devices.columnconfigure(5, weight=1)

        self._section_title(devices, "Company Devices").grid(row=0, column=0, columnspan=3, sticky=tk.W, padx=14, pady=(12, 2))
        ...
        self.network_health_panel_label.grid(row=0, column=3, columnspan=3, sticky=tk.E, padx=14, pady=(12, 2))
        self._muted_label(devices, textvariable=self.discovery_progress_var).grid(
            row=1,
            column=0,
            columnspan=3,
            sticky=tk.W,
        ...
        self._muted_label(devices, textvariable=self.last_scan_var).grid(
            row=1,
            column=3,
            columnspan=3,
            sticky=tk.E,
        ...
        self._muted_label(devices, text="Device Name").grid(row=2, column=0, sticky=tk.W, padx=(14, 8), pady=(0, 8))
        ttk.Entry(devices, textvariable=self.device_name_var, style="Dark.TEntry").grid(
            row=2,
            column=1,
            sticky=tk.EW,
            padx=(0, 12),
        ...
        self._muted_label(devices, text="IP Address / Host").grid(row=2, column=2, sticky=tk.W, padx=(0, 8), pady=(0, 8))
        ttk.Entry(devices, textvariable=self.device_address_var, style="Dark.TEntry").grid(
            row=2,
            column=3,
            sticky=tk.EW,
            padx=(0, 12),
        ...
        self._muted_label(devices, text="Device Type").grid(row=2, column=4, sticky=tk.W, padx=(0, 8), pady=(0, 8))
        ...
        device_type.grid(row=2, column=5, sticky=tk.EW, padx=(0, 14), pady=(0, 8))

        device_actions = tk.Frame(devices, bg=self.colors["panel"])
        device_actions.grid(row=3, column=0, columnspan=6, sticky=tk.EW, padx=14, pady=(0, 10))
        for column in range(7):
            device_actions.columnconfigure(column, weight=1)
        self._tool_button(device_actions, "Add Device", self.add_device, "Accent.TButton").grid(row=0, column=0, sticky=tk.EW, padx=(0, 6))
        self._tool_button(device_actions, "Remove Selected Device", self.remove_selected_device).grid(row=0, column=1, sticky=tk.EW, padx=6)
        self._tool_button(device_actions, "Save Devices", self.save_devices).grid(row=0, column=2, sticky=tk.EW, padx=6)
        self._tool_button(device_actions, "Load Devices", self.load_devices).grid(row=0, column=3, sticky=tk.EW, padx=6)
        self._tool_button(device_actions, "Check Company Network", self.check_company_network, "Accent.TButton").grid(row=0, column=4, sticky=tk.EW, padx=6)
        self.discover_button = self._tool_button(device_actions, "Discover Network Devices", self.discover_network_devices)
        self.discover_button.grid(row=0, column=5, sticky=tk.EW, padx=6)
        self.stop_discovery_button = self._tool_button(device_actions, "Stop Discovery", self.stop_discovery, "Danger.TButton")
        self.stop_discovery_button.grid(row=0, column=6, sticky=tk.EW, padx=(6, 0))
        self.stop_discovery_button.configure(state=tk.DISABLED)

        table_frame = tk.Frame(devices, bg=self.colors["panel"])
        table_frame.grid(row=4, column=0, columnspan=6, sticky=tk.EW, padx=14, pady=(0, 14))
        table_frame.columnconfigure(0, weight=1)

        columns = ("Name", "Address", "Type", "Last Status")
        self.devices_table = ttk.Treeview(table_frame, columns=columns, show="headings", height=5, style="Dark.Treeview")
        ...
        self.devices_table.tag_configure("ok", background="#10291d", foreground="#d1fae5")
        self.devices_table.tag_configure("down", background="#341216", foreground="#fee2e2")
        self.devices_table.tag_configure("unknown", background="#30230b", foreground="#fde68a")
        ...
        results = self._card(content)
        results.grid(row=5, column=0, sticky=tk.NSEW)
        results.rowconfigure(1, weight=1)
        results.columnconfigure(0, weight=1)

        results_header = tk.Frame(results, bg=self.colors["panel"])
        results_header.grid(row=0, column=0, sticky=tk.EW, padx=14, pady=(12, 8))
        results_header.columnconfigure(0, weight=1)
        self._section_title(results_header, "Results / Logs").grid(row=0, column=0, sticky=tk.W)
        self._tool_button(results_header, "Clear Output", self.clear_output).grid(row=0, column=1, sticky=tk.E, padx=(8, 0))
        self._tool_button(results_header, "Save Report", self.save_report).grid(row=0, column=2, sticky=tk.E, padx=(8, 0))
        self._tool_button(results_header, "Open Logs Folder", self.open_logs_folder, "Accent.TButton").grid(
            row=0,
            column=3,
            sticky=tk.E,
            padx=(8, 0),
        )
        self._muted_label(results_header, textvariable=self.status_var).grid(row=0, column=4, sticky=tk.E, padx=(14, 0))

        output_shell = tk.Frame(results, bg=self.colors["panel"])
